...
**Tests**: Manual acceptance only — the spec did not request an automated suite, and the event window
does not permit one. Every task carries the constitution principles it must satisfy.
...
- [ ] T004 [P] Write `package.json` — `"start": "node server.js"`, `"type": "module"`, zero dependencies
- [ ] T005 [P] Write `.env.example` with `ANTHROPIC_API_KEY=` and `PORT=` names only (Principle VI)
...
- [ ] T006 Write `data/cases.js` as an ES module exporting `CASES`, seeded from
...
- [ ] T007 Write `index.html` — semantic conversation region, result region, persistent synthetic and
  no-live-submission notices, mode banner, voice toggle (Principles I, VII)
- [ ] T008 Write `styles.css` — responsive at 375px and desktop, visible `:focus-visible` states,
...
- [ ] T009 [US1] Implement the five-question state machine in `app.js`; no step requests a personal
  identifier (FR-001, FR-002)
- [ ] T010 [US1] Implement the summary + explicit confirm gate; recommendation is unreachable before
  confirmation (FR-003, Principle IV)
- [ ] T011 [US1] Implement `matchCase()` deterministic scoring over `match_keywords` with a minimum
  confidence threshold (FR-011, FR-014)
- [ ] T012 [US1] Implement `renderResult()` — qualified recommendation wording, three-role
...
- [ ] T013 [US2] Verify CASE-03B routes to the state district office with the city-permit caveat and
  the school authority stakeholder role rendered (FR-005, FR-007)
- [ ] T014 [US2] Render the escalation path in the next action (FR-008)
...
- [ ] T015 [US3] Implement the `unresolved` render path — clarifying question or explicit decline,
...
- [ ] T016 Write `server.js` — `node:http` static file server plus `GET /api/health` and
  `POST /api/route`; loads `.env` manually; key never leaves the process (FR-017, Principle VI)
- [ ] T017 Write the constrained system prompt; model may return only a catalog `case_id`,
  `"unresolved"`, or a refusal (FR-015, Principle III)
- [ ] T018 Client-side validation — reject any `case_id` not present in `CASES`, fall back to the
  deterministic engine on any error or invalid payload (FR-015, FR-016)
- [ ] T019 Mode probe and banner — "Live Claude routing" vs "Simulated routing" (FR-014)

---

## Phase 7: Safety and voice

- [ ] T020 Static danger panel on the immediate-danger flag; no auto-dial, no `tel:` invocation;
  routing result still renders beneath (FR-012, FR-013, Principle II)
- [ ] T021 Web Speech input + readback behind a toggle; hidden when unsupported; text path unchanged;
...
- [ ] T022 Accessibility and responsive pass — keyboard-only run, focus visibility, 375px (FR-020)
- [ ] T023 Create public repo `AlyPosey/municipal-crosswalk-routing`, commit, push, enable Pages
- [ ] T024 Write `README.md` from `TEAM_README_TEMPLATE.md` — all sections, team ID marked TODO
- [ ] T025 Write `SUBMISSION.md` — copy-paste text for every Issue Form field
- [ ] T026 Final constitution audit: grep the repo for credentials, verify `is_synthetic` on every
